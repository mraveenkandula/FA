...
function can_process()
{
	...
	if (!check_num('output_qty', 0.0001))
	{
		display_error(_("Output quantity must be a positive number."));
		set_focus('output_qty');
		return false;
	}
	if ($_POST['input_item'] == $_POST['output_item'])
	{
		display_error(_("Input and output item cannot be the same."));
		return false;
	}
	return true;
}
...
{
	$woid = add_production_batch(
		$_POST['batch_date'], $_POST['stage'], $_POST['Location'],
		$_POST['input_item'], input_num('input_qty'),
	...
	display_notification(sprintf(_('Batch recorded as Work Order #%d. Yield: %s%%'), $woid, $yield));
	unset($_POST);
}
